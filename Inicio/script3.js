/*
    ╔══════════════════════════════════════════╗
    ║  Bahia Blanca Community                  ║
    ║  Desarrollado por: Reckless              ║
...
const sections = [
    {
        id: 0, icon: '<svg viewBox="0 0 24 24" width="18" height="18" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/></svg>', title: "Normas Generales",
        desc: "Reglas fundamentales que aplican a todos los jugadores en todo momento.",
        rules: [
            { id: "§1.1", title: "Respeto entre jugadores",
            ...
            { id: "§1.2", title: "Identidad y cuentas",
            ...
            { id: "§1.3", title: "Uso de modificaciones",
            ...
            { id: "§1.4", title: "Comportamiento fuera del juego",
            ...
            { id: "§1.5", title: "Acatamiento de decisiones del Staff",
    ...
    {
        id: 1, icon: '<svg viewBox="0 0 24 24" width="18" height="18" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M16 21v-2a4 4 0 0 0-4-4H6a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M22 21v-2a4 4 0 0 0-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/></svg>', title: "Roleplay",
        desc: "Normas para mantener la calidad e inmersión del roleplay en el servidor.",
        rules: [
            { id: "§2.1", title: "Metajuego (Metagaming)",
            ...
            { id: "§2.3", title: "Valor de vida (Fear RP)",
            ...
            { id: "§2.4", title: "Ruptura de personaje (Breaking Character)",
            ...
            { id: "§2.6", title: "Zonas seguras",
              text: "Las zonas seguras designadas son intocables para cualquier tipo de acción violentas o criminal. Está prohibido iniciar o continuar persecuciones, robos o enfrentamientos dentro de estas áreas." },
            { id: "§2.7", title: "Roleplay de muerte (RDM/VDM)",
    ...
    {
        id: 2, icon: '<svg viewBox="0 0 24 24" width="18" height="18" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M3 21h18"/><path d="M5 21V7l8-4v18"/><path d="M19 21V11l-6-4"/><path d="M9 9h1"/><path d="M9 13h1"/><path d="M9 17h1"/></svg>', title: "Organizaciones & Bandas",
        desc: "Reglas específicas para el funcionamiento de organizaciones criminales y legales.",
        rules: [
            { id: "§3.1", title: "Registro de organizaciones",
            ...
            { id: "§3.2", title: "Límite de miembros",
            ...
            { id: "§3.3", title: "Guerras entre organizaciones",
    ...
    {
        id: 3, icon: '<svg viewBox="0 0 24 24" width="18" height="18" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="12" y1="1" x2="12" y2="23"/><path d="M17 5H9.5a3.5 3.5 0 0 0 0 7h5a3.5 3.5 0 0 1 0 7H6"/></svg>', title: "Economía & Trabajos",
        desc: "Normativa sobre el uso del sistema económico y los trabajos disponibles.",
        rules: [
            { id: "§4.1", title: "Explotación de bugs económicos",
            ...
            { id: "§4.2", title: "Transferencias y comercio",
            ...
            { id: "§4.3", title: "Trabajos ilegales",
    ...
    {
        id: 4, icon: '<svg viewBox="0 0 24 24" width="18" height="18" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"/></svg>', title: "Policía & Servicios de Emergencia",
        desc: "Normas para los jugadores que forman parte de cuerpos de seguridad y emergencias.",
        rules: [
            { id: "§5.1", title: "Conducta policial",
            ...
            { id: "§5.3", title: "Servicios médicos",
    ...
    {
        id: 5, icon: '<svg viewBox="0 0 24 24" width="18" height="18" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M10.29 3.86L1.82 18a2 2 0 0 0 1.71 3h16.94a2 2 0 0 0 1.71-3L13.71 3.86a2 2 0 0 0-3.42 0z"/><line x1="12" y1="9" x2="12" y2="13"/><line x1="12" y1="17" x2="12.01" y2="17"/></svg>', title: "Sistema de Sanciones",
        desc: "Procedimiento y escala de sanciones aplicadas por el equipo de administración.",
        rules: [
            { id: "§6.1", title: "Escala de sanciones",
            ...
            { id: "§6.2", title: "Proceso de apelación",
            ...
            { id: "§6.3", title: "Sanciones inmediatas",
...
function buildNav() {
    ...
    navContainer.innerHTML = sections.map((s, i) => `
        <div class="sidebar__nav-item ${i === 0 ? 'sidebar__nav-item--active' : ''}" data-section="${s.id}" onclick="navigateTo(${s.id})">
            <div class="sidebar__nav-icon">${s.icon}</div>
            <div class="sidebar__nav-info">
                <div class="sidebar__nav-title">${s.title.length > 22 ? s.title.slice(0,22)+'…' : s.title}</div>
                <div class="sidebar__nav-count">${s.rules.length} normas</div>
            </div>
            <div class="sidebar__nav-number">${String(i+1).padStart(2,'0')}</div>
        </div>
    `).join("");
}
...
function renderContent(filter = "") {
    ...
    sections.forEach(section => {
        ...
        const rulesHTML = filtered.map(function(rule) {
            return '<div class="rule">' +
                '<div class="rule__header">' +
                    '<span class="rule__id">' + rule.id + '</span>' +
                    '<span class="rule__title">' + highlight(rule.title, filter) + '</span>' +
                '</div>' +
                '<div class="rule__divider"></div>' +
                '<div class="rule__body">' +
                    '<div class="rule__border"></div>' +
                    '<p class="rule__text">' + highlight(rule.text, filter) + '</p>' +
                '</div>' +
            '</div>';
        }).join("");

        box.innerHTML = '<div class="section__header">' +
            '<div class="section__icon">' + section.icon + '</div>' +
            '<div class="section__info">' +
                '<div class="section__name">' + section.title + '</div>' +
                '<div class="section__desc">' + section.desc + '</div>' +
            '</div>' +
        '</div>' +
        '<div class="section__rules">' +
            rulesHTML +
        '</div>';
    ...
    if (!any) {
        container.innerHTML = '<div class="empty">' +
            ...
            '<p class="empty__text">No se encontraron normas para <strong>"' + esc(filter) + '"</strong></p>' +
        '</div>';
    }
}

/* Navigation & Interactive Systems */
...
function setActive(id) {
    document.querySelectorAll(".sidebar__nav-item").forEach(n =>
        n.classList.toggle("sidebar__nav-item--active", parseInt(n.dataset.section) === id));
}
